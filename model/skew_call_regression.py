...
def get_k_and_close(i, j):
    ...
    sql = f"SELECT k,CLOSE\nfrom OPTION_LOCAL ol\nwhere " \
          f"TRADE_DT ={j} and CALL_PUT='call' and OPTION_ENDTRADE={i} " \
...
def seasonal_skew_call(end_date_all):
    R = 0.02
...
def skew_call_regression():
    # 一年十二个月，每一个月都有一个期权到期日，针对四年共48个到期日，计算出所有天数对应的iv_diff
    end_date_all = get_end_date(20160323, 20200226)
    iv_diff_all_dict, iv_diff_all_list, days_to_maturity_list = seasonal_skew_call(end_date_all)
    ...
    iv_diff_up_middle_low_pd.to_csv("E:\\XX\\option_arbitrage\\data\\mid\\csv\\skew_call_up_middle_low.csv")
    print(iv_diff_up_middle_low_pd)


if __name__ == "__main__":
    skew_call_regression()